...
port(
	CLK           : in  std_logic;
	cpu_cen       : in  std_logic := '1';   -- TOOBIN: clock enable; gate the whole CPU to run at CLK/N (single clock domain)
	RESET         : in  std_logic;
	HALT          : in  std_logic;
...
PROCESS (cpu_cen, state, clkena_e, skipFetch)
BEGIN
	IF cpu_cen='1' AND (state="01" OR clkena_e='1' OR skipFetch='1') THEN
		clkena <= '1';
	ELSE
		clkena <= '0';
	END IF;
